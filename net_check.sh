...
network_netstat_port_by_pid() {
    ...
    sudo netstat -nap 2>/dev/null | sed -E 's/\[\s*ACC\s*\]/[ACC]/g; s/                                                    \[\s*\]/[]/g' | tr -s ' ' | grep -E "tcp|udp" | grep "$pid/" | grep -v                                                     "NetworkManager" | grep -v "dhclient" | while read -r line; do
        ...
        if [[ "$sixth" =~ ^[0-9]+/.*$ ]]; then
            state="-"
            pid_info=$(echo "$line" | awk '{for (i=6; i<=NF; i++) print                                                    f $i " "; print ""}')
        else
            state="$sixth"
            pid_info=$(echo "$line" | awk '{for (i=7; i<=NF; i++) print                                                    f $i " "; print ""}')
        fi
        ...
        gen_log "[PORT] PID=$pid_field | 프로세스=$pname_field | 프로토                                                    콜=$proto | 포트=${laddr} → ${raddr}"
    done
}
...
network_ss_check() {
    ...
    sudo ss -0pb 2>/dev/null | paste - - | while read -r line; do
        if echo "$line" | grep -qE '21139|29269|36204|40783|0x5293|0x72                                                    55|0x39393939|0x8D6C|0x9F4F|5353|262144'; then
            ...
            gen_log "[WARN] ss: 의심 연결 감지 → PID=$pid | 프로세스=$p                                                    name | $laddr → $raddr"
...
network_lsof_check() {
    ...
    for pid in $pids; do
        ...
        gen_log "[WARN] RAW/DGRAM 사용 프로세스 감지 → PID=$pid | 프로                                                    세스=$cmd | 실행 경로=$exe_path"
