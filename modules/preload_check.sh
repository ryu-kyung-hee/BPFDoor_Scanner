...
check_ld_preload() {
    ...
    if [ -f "/etc/ld.so.preload" ]; then
        if [ -s "/etc/ld.so.preload" ]; then
            gen_log "${RED}WARN: /etc/ld.so.preload 파일에 내용이 존재함${NC}"
            ...
            gen_log "INFO: /etc/ld.so.preload 파일은 존재하지만 비어 있음"
        fi
    fi

    if [ "$found" = true ]; then
        gen_log "${RED}WARN: LD_PRELOAD 관련 이상 징후 감지됨${NC}"
    else
        gen_log "${GREEN}INFO: LD_PRELOAD 관련 의심 정황 없음${NC}"
    fi
}
